...
class Spider():
    ...
    def __save_to_csv(self,anchors,file_name,urlsFileCsv):
        with open(os.getcwd()+'/result/'+file_name[0:-4]+'.csv','w',encoding='gbk',errors="ignore",newline='') as f:
            ...
            for anchor in anchors:
                i=i+1
                #此处增加一个小key,如果为0则说明来自淘宝，如果是1则说明是来自天猫,'#'为第二个步骤的解析标识符
                if anchor['url'].find(self.FROM_TAOBAO)!=-1:
                    urlsFileCsv.writerow({'fileName':file_name,'url':anchor['url'],'rowNum':str(i)+"#"+"0"})
                elif anchor['url'].find(self.FROM_TIANMAO)!=-1:
                    urlsFileCsv.writerow({'fileName':file_name,'url':anchor['url'],'rowNum':str(i)+"#"+"1"})
                else:
                    #如果url即不属于淘宝也不属于天猫,则直接跳过不处理（算是异常数据）
                    pass
    ...
    def go(self):
        ...
        urlsFile=open(os.getcwd()+'/temp/'+'fileTemp.csv','w',encoding='gbk',errors="ignore",newline='')
